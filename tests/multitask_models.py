...
from ecodyna.models.mutitask_models import MultiTaskTimeSeriesModel, MyRNN, MyNBEATS, MyTransformer
from models.task_modules import ChunkClassifier

common_parameters = dict(n_in=5, space_dim=8)
models = {
    'GRU': (MyRNN, {'model': 'GRU', 'n_layers': 2, 'n_hidden': 12}),
    'LSTM': (MyRNN, {'model': 'LSTM', 'n_layers': 2, 'n_hidden': 12}),
    'N-BEATS': (
        MyNBEATS, {'layer_widths': 32, 'n_layers': 4, 'expansion_coefficient_dim': 5, 'n_stacks': 2, 'n_blocks': 2}),
    'Transformer': (MyTransformer, {'nhead': common_parameters['space_dim']})
}
...
class MultiTaskTimeSeriesModelsTests(unittest.TestCase):
    ...
    def test_default_featurizer(self):
        for Model, params in models.values():
